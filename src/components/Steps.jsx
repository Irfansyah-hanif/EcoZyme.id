...
export default function Steps({ activeStep = 0, setActiveStep }) {
  const safeIndex = (typeof activeStep === 'number' && activeStep >= 0 && activeStep < stepsData.length)
    ? activeStep
    : 0;

  const currentStep = stepsData[safeIndex] || {};
              ...
              return (
                ...
                >
                  <div className={`w-9 h-9 sm:w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm sm:text-lg flex-shrink-0 ${
                    safeIndex === index ? 'bg-white/20 text-white' : 'bg-emerald-100 text-emerald-700'
                  }`}>
                    {index + 1}
                  </div>
                  ...
                  <div className={`w-7 h-7 sm:w-8 h-8 rounded-full flex items-center justify-center transition-transform ${safeIndex === index ? 'bg-white/10 text-white' : 'text-emerald-600'}`}>
                    {IconComponent && <IconComponent className="w-4 h-4 sm:w-5 h-5" />}
                  </div>
                </button>
              );
            })}
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div className="h-1 bg-slate-200 flex-grow rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${((safeIndex + 1) / stepsData.length) * 100}%` }}
                  ></div>
                </div>
              </div>
