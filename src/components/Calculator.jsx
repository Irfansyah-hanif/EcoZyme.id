...
export default function Calculator({ wasteAmount, setWasteAmount }) {
  ...
  // MODIFIKASI: Menambah kelipatan 1 gram
  const handleIncrement = () => {
    setWasteAmount((prev) => (prev || 0) + 1);
  };

  // MODIFIKASI: Mengurangi kelipatan 1 gram (minimal 0)
  const handleDecrement = () => {
    setWasteAmount((prev) => Math.max(0, (prev || 0) - 1));
  };

  return (
    <section id="kalkulator" className="py-16 md:py-20 bg-slate-50 border-t border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <span className="text-emerald-600 font-bold uppercase tracking-widest text-sm bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">Alat Ukur Akurat</span>
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden max-w-4xl mx-auto border border-slate-100">
          <div className="p-6 sm:p-8 md:p-12">
            
            {/* Preset Buttons */}
            <div className="mb-8">
              <span className="block text-xs font-bold text-slate-400 uppercase tracking-widest text-center mb-3">Pilih Preset Cepat Sampah</span>
              <div className="flex flex-wrap gap-2 justify-center">
                {[150, 300, 500, 1000, 3000, 5000].map((amt) => (
                  <button
                    key={amt}
                    onClick={() => setWasteAmount(amt)}
            ...
            <div className="mb-12 text-center">
              ...
              <div className="relative max-w-xs mx-auto flex items-end justify-center border-b-4 border-slate-200 focus-within:border-emerald-500 transition-colors pb-2 px-2">
                <input
                  type="number"
                  min="0"
                  value={wasteAmount === 0 ? '' : wasteAmount}
                  onChange={(e) => {
                    const val = e.target.value;
                    setWasteAmount(val === '' ? 0 : Math.max(0, parseFloat(val)));
                  }}
                ...
                {/* Bagian Tulisan Gram & Tombol Pengatur Kustom */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="bg-amber-50 rounded-2xl p-6 text-center border border-amber-100">
                <div className="mx-auto w-14 h-14 bg-amber-100 rounded-xl flex items-center justify-center mb-4"><FlaskConical className="w-7 h-7 text-amber-600" /></div>
              ...
              <div className="bg-emerald-50 rounded-2xl p-6 text-center border border-emerald-100">
                <div className="mx-auto w-14 h-14 bg-emerald-100 rounded-xl flex items-center justify-center mb-4"><Leaf className="w-7 h-7 text-emerald-600" /></div>
              ...
              <div className="bg-blue-50 rounded-2xl p-6 text-center border border-blue-100">
                <div className="mx-auto w-14 h-14 bg-blue-100 rounded-xl flex items-center justify-center mb-4"><Droplets className="w-7 h-7 text-blue-600" /></div>
            ...
            <div className="mt-8 p-5 sm:p-6 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl border border-emerald-100 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex gap-3 items-start">
                <div className="bg-white p-2 rounded-xl shadow-sm text-emerald-600 flex-shrink-0"><Info className="w-5 h-5" /></div>
                <div>
              ...
              <div className="bg-white px-6 py-3 rounded-xl border border-emerald-200 text-center w-full md:w-auto shadow-sm">
